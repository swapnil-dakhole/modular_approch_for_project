...
$(document).ready(function () {
  let response = ProductService.getProductDetails()
    .then((resp) => {
      ...
      // console.log(resp.data[0]._productName);
      //   return resp;
      $("#productDetails").html(`<table class='prod_table'></table>`);
      $(".prod_table").html(
        `<tr><th>ID</th><th>Product Name</th><th>Brand Name</th><th>Price</th><th>Availbility</th><th>Image</th><th>Description</th></tr>`
      );
      $(".card-container").html();
      $(".card").html();
    ...
    <tr class='table-row' id="table-row">
        ...
        <td><button  class="btn" productId="${prod.id}" >Delete</button></td>
    </tr>`;
